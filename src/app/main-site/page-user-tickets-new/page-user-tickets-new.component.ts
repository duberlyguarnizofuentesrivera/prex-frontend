...
export class PageUserTicketsNewComponent implements OnInit {
  ...
  constructor() {
    ...
    this.regions = Object.keys(this.cuidadesPeru);
    this.provinces = [];
    this.districts = [];
  ...
  searchRegion(event: { query: string; }) {
    this.regionResults = this.regions.filter(region => region.toLowerCase().indexOf(event.query.toLowerCase()) == 0);
  }

  searchProvince(event: { query: string; }) {
    // @ts-ignore
    this.provinces = Object.keys(this.cuidadesPeru[(this.region)]);
    this.provinceResults = this.provinces.filter(province => province.toLowerCase().indexOf(event.query.toLowerCase()) == 0);
  }

  searchDistrict(event: { query: string; }) {
    // @ts-ignore
    this.districts = Object.keys(this.cuidadesPeru[(this.region)][(this.province)]);
    this.districtResults = this.districts.filter(district => district.toLowerCase().indexOf(event.query.toLowerCase()) == 0);
  }

  validateCity() {
    this.validCitySelection = false;
    if (this.regions.includes(this.region)) {
      // @ts-ignore
      const selectedProvince = this.cuidadesPeru[(this.region)]
      if (selectedProvince !== undefined && selectedProvince !== null) {
        this.provinces = Object.keys(selectedProvince);
        // @ts-ignore
        const selectedDistrict = this.cuidadesPeru[(this.region)][(this.province)];
        if (selectedDistrict !== undefined && selectedDistrict !== null) {
          this.districts = Object.keys(selectedDistrict);
          this.validCitySelection = this.provinces.includes(this.province) && this.districts.includes(this.district);
        }
      }
    }
